...
class BookingCalendarSync:
    ...
    def __init__(self, session: AsyncSession, resolver: CalendarClientResolver):
        self.session = session
        self.resolver = resolver

    async def created(self, appointment_id: int) -> None:
        """Create and link an event after the booking transaction commits."""
        ...
        try:
            async with self.session.begin():
                ...
                if business is None:
                    return
                resolved = await self.resolver.resolve(appointment.business_id)
                if resolved is None:
                    return
            ...
            logger.warning("Calendar creation synchronization failed for appointment %s (%s)",
    ...
    async def cancelled(self, appointment_id: int) -> None:
        ...
        try:
            ...
            calendar = CalendarService(resolved.client)
            await calendar.sync_cancelled_appointment(
                appointment=appointment,
                business=business,
                calendar_id=resolved.calendar_id,
            )
        ...
        try:
            async with self.session.begin():
                ...
                for obj in (appointment, business, service, customer):
                    self.session.expunge(obj)
            calendar = CalendarService(resolved.client)
            await calendar.sync_rescheduled_appointment(
                appointment=appointment,
                business=business,
                service=service,
                customer=customer,
